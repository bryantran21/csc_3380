...
public class Schedule implements ActionListener, ItemListener {
	...
	public Schedule(User passedUser) {
		...
		for (int i = 0; i < 7; i++) // Updates the radio buttons displayed on the GUI to the previously selected
									// availabilities (if they said Monday was available, Monday will be
									// preselected)
		{
		...
		int counter = 0;
		while (currentUser.getSchedule().week[counter].availability.equals("unavailable") && counter < 6) {		// Increments a counter variable up until the first selected available day
			if (counter < 6) {
				counter++;
			}
		}
		if (counter == 6 && currentUser.getSchedule().week[counter].availability.equals("unavailable"))			// If the counter reaches 6 and Sunday is listed as unavailable, updates dayAvailable to empty
			dayAvailable = "";
		else {		// Updates dayAvailable to the first chosen day available (if Wednesday is the first selected then writes 'Wednesday')
			dayAvailable = currentUser.getSchedule().week[counter].dayName;
		}
		for (int i = counter + 1; i < 7; i++) {		// Populates the rest of dayAvailable string accordingly
			if ((currentUser.getSchedule().week[i].availability.equals("available"))) {
				dayAvailable = dayAvailable + ", " + currentUser.getSchedule().week[i].dayName;
			}
		}
		if (dayAvailable.equals("")) {		// if the string is populated and empty, displays that user has not selected any days
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == homeButton) {		// home button to bring user to home page

			HomePage home = new HomePage(currentUser);
			frame.dispose();
		}
		if (e.getSource() == logoutButton) {	// logout button to log user out

			LoginGui login = new LoginGui();
			frame.dispose();
		}

		if (e.getSource() == updateBtn) {		// update button to update the availability of the user

			int counter = 0;
			while (dayArray[counter].equals("") && counter < 6) {	// Increments a counter variable up until the first selected available day
				if (counter < 6) {
					counter++;
				}
			}
			daySelect = dayArray[counter];
			for (int i = counter + 1; i < 7; i++) {					// Builds a string based on the dayArray to be able to display in JLabel
		...
		if (e.getSource() == confirmBtn) { 			// Button to confirm the user's actions and updates the user's availability
			String availability = "";
			for (int i = 0; i < 7; i++) {			// Goes through each index in the dayArray and update's the user's availability for each day
				if ((dayArray[i].compareTo("") != 0)) {
				...
				switch (i) {		// Switch case to compare each array index to string
				case 0:
					dayParameter = "Monday";
					break;
				case 1:
				...
				setAvailability(currentUser.getEmail(), dayParameter, availability);	// updates the user's availability for day at index i
			}
			currentUser = returnUser(currentUser.getEmail());
		...
		if (e.getSource() == declineBtn) {		// closes the dialogue box
	...
	@Override
	public void itemStateChanged(ItemEvent e) {		// Changes the string stored in specific index of dayArray based on the radio buttons being selected or unselected
		daySelect = "";
